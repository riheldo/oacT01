...
#			Trabalho 1 - Programacao Assembler
#
# Nome: Ana Luisa Salvador Alvarez	Matricula: 16/0048036 
# Nome: Pedro Lucas Pinto Andrade	Matricula: 16/0038316 
# Nome: Riheldo de Melo			Matricula: 11/0138899

.data

image_name:		.asciiz "enaeye.raw"
...
#MENU 
#Interface inicial com o usuário, utilizando chamadas de sistema. 
#A interface inicial do usuário contem um cardápio de opções e servir como um guia de ajuda de como utilizar cada função
...
#FUNCAO GET POINT
#Essa função tem como parâmetros uma posição dada por x e y. 
#Como retorno, a função imprime na console os valores dos componentes R, G e B do ponto indicado pela posição dada por x e y.	
...
#FUNCAO DRAW POINT
#Essa função tem como parâmetros uma posição dada por x e y, além de um valor val em RGB. 
#Para a formação do valor val, é solicitado ao usuário separadamente os valores das componentes R, G e B (através de números em decimal entre 0 e 255).
# Como retorno, a função desenha um ponto de cor definida pelo valor val na posição denotada por x e y do Bitmap Display.	
...
#FUNCAO DRAW EMPTY RECTANGLE
#Essa função tem como parâmetros duas posições, sendo uma dada por xi e yi e outra dada por xf e yf; além de um valor val em RGB. 
#Para a formação do valor val, é solicitado ao usuário separadamente os valores das componentes R, G e B
#(através de números em decimal entre 0 e 255). 
#Como retorno é desenhado um retângulo sem preenchimento de cor, ou seja, com bordas de cor val e tamanho de um ponto. 
#Um dos quatro cantos do retângulo desenhado é o ponto de posição xi e yi; e outro canto, o ponto de posição xf e yf.	
...
#FUNCAO LOAD IMAGE
#Essa função tem como parâmetros o nome de uma imagem image_name a ser carregada no display, 
#o endereço base de memória do display address, 
#o endereço de uma palavra na memoria para utilizar como buffer e o tamanho size da imagem em pixels. 
#A função retorna a exibição da imagem no bitmap display.	
...
# FUNCAO CONVERT NEGATIVE 
# Essa função não possui parâmetros. Como retorno, essa função apresenta o negativo da imagem contida no Bitmap Display. 
#O negativo de uma imagem é obtido através da inversão de cores de uma imagem normal.
# Para fazer isso, obtem-se o componente R, G e B de cada ponto do Bitmap Display e subtrai-se de 255
#(O novo valor RGB de um ponto é 255 – R, 255 –G e 255 – B).
